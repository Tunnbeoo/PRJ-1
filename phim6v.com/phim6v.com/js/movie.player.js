  ...
  (function (a) {
    ...
    a.html5.view = function (n, H, h) {
      ...
      function L() {
        ...
        function U() {
          if (
            m.skin.getComponentSettings("display") &&
      ...
      function K() {
        for (var U = 0; U < j.plugins.order.length; U++) {
          ...
          if (d.exists(j.plugins.object[V].getDisplayElement)) {
            j.plugins.object[V].height = d.parseDimension(
              j.plugins.object[V].getDisplayElement().style.height
            );
            j.plugins.object[V].width = d.parseDimension(
              j.plugins.object[V].getDisplayElement().style.width
            );
        ...
        if (d.exists(X)) {
          ...
          for (var U = 0; U < j.plugins.order.length; U++) {
            ...
            if (d.exists(j.plugins.object[W].getDisplayElement)) {
              j.plugins.config[W].currentPosition =
                j.plugins.config[W].position;
            }
          }
        }
        G(j.width, j.height);
      ...
      function M(U) {
        switch (U.keyCode) {
          ...
          case 32:
            if (
              m.jwGetState() != a.api.events.state.IDLE &&
      ...
      function G(U, ad) {
        ...
        if (!j.fullscreen) {
          ...
          if (typeof U == "string" && U.indexOf("%") > 0) {
            g =
              (d.getElementWidth(d.parentNode(R)) *
                parseInt(U.replace("%"), "")) /
              100;
          } else {
            g = U;
          ...
          if (typeof ad == "string" && ad.indexOf("%") > 0) {
            t =
              (d.getElementHeight(d.parentNode(R)) *
                parseInt(ad.replace("%"), "")) /
              100;
          } else {
            t = ad;
      ...
      function w(ab, X, Y) {
        ...
        for (var W = 0; W < X.length; W++) {
          ...
          if (d.exists(j.plugins.object[aa].getDisplayElement)) {
            if (
              j.plugins.config[aa].currentPosition !=
              a.html5.view.positions.NONE
            ) {
              var U = ab(aa, F--);
              if (!U) {
                Z.push(aa);
      ...
      var l = (this.resizeMedia = function () {
        ...
        if (W && W.tagName.toLowerCase() == "video") {
          ...
          if (V) {
            j.getMedia().resize(
              d.parseDimension(V.style.width),
              d.parseDimension(V.style.height)
            );
          } else {
            j.getMedia().resize(
              d.parseDimension(o.style.width),
      ...
      var z = (this.getComponentPosition = function (V) {
        ...
        switch (U.toUpperCase()) {
          case a.html5.view.positions.TOP:
            ...
            o.style[U] =
              d.parseDimension(o.style[U]) + j.plugins.object[V].height + "px";
          ...
          case a.html5.view.positions.LEFT:
            ...
            W.height =
              ...
              d.parseDimension(o.style.bottom);
            o.style[U] =
              d.parseDimension(o.style[U]) + j.plugins.object[V].width + "px";
            o.style.width = d.getElementWidth(o) - W.width + "px";
            break;
          default:
            break;
  ...
  (function (a) {
    ...
    a.html5.controlbar = function (m, Y) {
      ...
      function y(aq, au, ao, ax, ar, an, ap) {
        if (
          _utils.exists(l.skin.getSkinElement("controlbar", aq)) ||
          aq.indexOf("Text") > 0 ||
          ...
          if (
            (aq.indexOf("next") === 0 || aq.indexOf("prev") === 0) &&
          ...
          if (q) {
            ...
            var aw = ad(aq, ax, at);
            if (
              _utils.exists(l.skin.getSkinElement("controlbar", aq + "Over"))
            ) {
              aw.onmouseover = function (az) {
                aw.style.backgroundImage = [
                  "url(",
                  l.skin.getSkinElement("controlbar", aq + "Over").src,
                  ")",
                ].join("");
              };
      ...
      function v(ao) {
        ...
        if (c.timeSliderRail) {
          ...
          var an = isNaN(Math.round((ap * g) / 100))
            ? 0
            : Math.round((ap * g) / 100);
      ...
      function s(an) {
        if (
          an.newstate == a.api.events.state.BUFFERING ||
      ...
      function J(at) {
        ...
        if (av) {
          var ap = isNaN(Math.round(av.width * aq))
            ? 0
            : Math.round(av.width * aq);
      ...
      function o() {
        ...
        for (var ap = 0; ap < an.length; ap++) {
          ...
          for (var ao in ar) {
            ...
            if (
              ar[ao].id.indexOf(ag.id + "_divider") === 0 &&
              aq &&
        ...
        if (_utils.exists(S.volumeSliderRail)) {
          ...
          if (S.volumeSliderThumb) {
            var ar =
              an -
              Math.round(
                _utils.parseDimension(S.volumeSliderThumb.style.width) / 2
              );
            ar = Math.min(
              Math.max(ar, 0),
              aq - _utils.parseDimension(S.volumeSliderThumb.style.width)
            );
  ...
  (function (a) {
    ...
    a.html5.display = function (k, K) {
      ...
      function B(Y) {
        ...
        c();
        if (
          h.jwGetState() == a.api.events.state.IDLE ||
  ...
  (function (b) {
    ...
    b.html5.mediavideo = function (k, I) {
      ...
      function Q(Y) {
        ...
        if (f.exists(Y) && f.exists(Y.target)) {
          ...
          if (h == b.api.events.state.PLAYING) {
            ...
            l.position =
              l.duration > 0 ? Math.round(Y.target.currentTime * 10) / 10 : 0;
            u(b.api.events.JWPLAYER_MEDIA_TIME, {
              position: l.position,
              duration: l.duration,
            });
            if (
              l.position >= l.duration &&
      ...
      function q(Z) {
        ...
        if (
          (Z.target.error && Z.target.tagName.toLowerCase() == "video") ||
          (Z.target.parentNode.error &&
            Z.target.parentNode.tagName.toLowerCase() == "video")
        ) {
          var X = !f.exists(Z.target.error)
            ? Z.target.parentNode.error
            : Z.target.error;
          switch (X.code) {
