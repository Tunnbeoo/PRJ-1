/*! jQuery v1.7.2 jquery.com | vuhoahay movie 1.0 */
    ...
    settings = jQuery.extend(
      {
        ...
        imageLoading: "/contents/img/lightbox-ico-loading.gif", // (string) Path and the name of the loading icon

        imageBtnPrev: "/contents/img/lightbox-btn-prev.gif", // (string) Path and the name of the prev button image

        imageBtnNext: "/contents/img/lightbox-btn-next.gif", // (string) Path and the name of the next button image

        imageBtnClose: "/contents/img/lightbox-btn-close.gif", // (string) Path and the name of the close btn

        imageBlank: "/contents/img/lightbox-blank.gif", // (string) Path and the name of a blank image (one pixel)
...
/* jQuery Carousel 0.9.8 | vuhoahay movie 1.0 ichphienpro*/
...
(function (g) {
  g.fn.carousel = function (q) {
    ...
    return this.each(function () {
      ...
      r.$elts.content = g(this)
        .children()
        .css({ position: "absolute", top: 0 });
      r.$elts.wrap = r.$elts.content
        .wrap('<div class="carousel-wrap"></div>')
        .parent()
        .css({ overflow: "hidden", position: "relative" });
      r.steps = { first: 0, count: r.$elts.content.children().length };
  ...
  function c(s) {
    ...
    if (s.params.direction == "vertical") {
      ...
      s.$elts.content
        .parent()
        .css({ height: s.itemHeight * s.params.dispItems + "px" });
    } else {
      s.$elts.content.parent().css({ height: s.itemHeight + "px" });
    }
    i(s);
  }
  function d(q) {
    q.$elts.nextBtn
      ...
      .bind("enable", function () {
        ...
        if (q.params.combinedClasses) {
          r.removeClass("next-disabled previous-disabled").removeAttr(
            "disabled"
          );
        }
      })
  ...
  function o(q) {
    q.$elts.pagination = g(
      ...
      [q.params.paginationPosition == "outside" ? "insertAfter" : "appendTo"](
        q.$elts.carousel
      )
      .find("p");
    ...
    if (r.params.pagination) {
      r.$elts.paginationBtns = r.$elts.paginationBtns
        .add(
          g(
            '<a role="button"><span>' +
              r.params.tabLabel(r.$elts.paginationBtns.length + 1) +
              "</span></a>"
          ).data("firstStep", q)
        )
        .appendTo(r.$elts.pagination);
    ...
    if (q == "prev") {
      ...
      if (q == "next") {
        if (r.steps.first + r.params.dispItems < r.steps.count) {
          if (!r.params.showEmptyItems) {
            return Math.min(
              r.steps.first + r.params.dispItems,
    ...
    switch (q.params.effect) {
      ...
      case "fade":
        if (q.params.direction == "vertical") {
          q.$elts.content
            .hide()
            .css("top", -(q.itemHeight * r) + "px")
            .fadeIn(q.params.animSpeed);
        } else {
          q.$elts.content
            .hide()
            .css("left", -(q.itemWidth * r) + "px")
            .fadeIn(q.params.animSpeed);
        }
        break;
      default:
  ...
  function a(r) {
    ...
    q.bind({
      play: function () {
        ...
        q.removeClass("pause")
          .addClass("play")
          .html(r.params.stopSlideTextPause);
      },
      pause: function () {
        m(r);
        q.removeClass("play")
          .addClass("pause")
          .html(r.params.stopSlideTextPlay);
      },
    });
    q.click(function (s) {
      if (q.is(".play")) {
...
function alert_showmsg(msgs) {
  ...
  jAlert(whole_msg, "Thông báo lỗi");
...
function TestEmail(objValue, strError) {
  ...
  if (objValue.value.length > 0 && !validateEmail(objValue.value)) {
    if (!strError || strError.length == 0) {
      strError = objValue.name + ": Vui lòng nhập đúng email ";
    } //if
  ...
  switch (command) {
    ...
    case "numeric": {
      ret = TestInputType(
        ...
        objValue.name + ": Dữ liệu phải là số "
      );
