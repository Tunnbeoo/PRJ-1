...
(function (_V_) {
    ...
    extend: function (destination, source) {
      for (var prop in source) {
        ...
        if (typeof source[prop] == "object" && null !== source[prop]) {
          destination[prop] = methods.extend(
            destination[prop] || {},
            source[prop]
          );
        } else {
  ...
  _V_.ResolutionMenuItem = _V_.MenuItem.extend({
    ...
    init: function (player, options) {
      ...
      options.selected =
        options.res.toString() === player.getCurrentRes().toString();
  ...
  _V_.ResolutionMenuItem.prototype.onClick = function () {
    ...
    if (player.getCurrentRes() == this.resolution) {
      return;
    }
    ...
    player
      .src(player.availableRes[this.resolution])
    ...
    while (button_node_count > 0) {
      ...
      if ("vjs-current-res" == button_nodes[button_node_count].className) {
        button_nodes[button_node_count].innerHTML = methods.res_label(
          this.resolution
        );
        break;
      }
  ...
  _V_.ResolutionSelector.prototype.createItems = function () {
    ...
    items.push(
      new _V_.ResolutionTitleMenuItem(player, {
    ...
    for (current_res in player.availableRes) {
      ...
      items.push(
        new _V_.ResolutionMenuItem(player, {
          res: current_res,
        })
      );
    }
  ...
  _V_.plugin("resolutionSelector", function (options) {
    ...
    var player = this,
      ...
      default_resolutions =
        settings.default_res && typeof settings.default_res == "string"
          ? settings.default_res.split(",")
          : [];
      ...
      for (current_res in available_res) {
        ...
        while (i > 0) {
          ...
          while (j > 0) {
            ...
            if (
              settings.force_types[i] === available_res[current_res][j].type
            ) {
              found_types++;
            }
    ...
    resolutionSelector = new _V_.ResolutionSelector(player, {
      el: _V_.Component.prototype.createEl(null, {
        ...
        innerHTML:
          '<div class="vjs-control-content"><span class="vjs-current-res">' +
          (current_res || "Quality") +
          "</span></div>",
        role: "button",
    ...
    player.controlBar.resolutionSelector =
