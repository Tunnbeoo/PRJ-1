...
--
-- Máy chủ: 127.0.0.1
-- Thời gian đã tạo: Th10 25, 2024 lúc 01:47 AM
...
(1, 'nguyenvana', 'nguyenvana@gmail.com', '0123465789', 'KHong', 'KHông có gì', '2023-04-13 20:40:00'),
...
(93, 'THEPHONERSTORE242113', 671116765, 3056150.00, 61010615.00, 'Thanh toán khi nhận hàng', 11, 'Trần Nhật Sang', '0937988510', 'nhatsang0101@gmail.com', '19/7c Đông Tác, Phường Tân Đông Hiệp, Thành phố Dĩ An, Bình Dương', 'dsadsadas', '2023-04-09 14:31:43', 6, 0, '', '                hủy'),
...
(1, 'Điện thoại OPPO Reno8 T 5G 256GB', 10999000.00, 10, 'oppo - reno 5g 8t den.png,oppo-reno8t-den1-thumb-600x600.jpg,oppo-reno-5g-dep.jpg,638091158554197072_oppo-reno8-t-5g-dd.jpg,Oppo Reno 5G.png,oppo-reno-8t.jpg,thumb-oppo-reno8t-vang1-thumb-600x600.jpg', 5.00, 1, 19, '2023-04-01 09:20:03', '0000-00-00 00:00:00', '2024-10-22 20:34:57', 0, 1, '<p>OPPO Reno8 T 5G 128GB l&agrave; mẫu điện thoại đầu ti&ecirc;n trong năm 2023 m&agrave; OPPO kinh doanh tại Việt Nam. M&aacute;y nhận được kh&aacute; nhiều sự quan t&acirc;m đến từ cộng đồng c&ocirc;ng nghệ về th&ocirc;ng số kỹ thuật hết sức ấn tượng như: Camera 108 MP, chipset nh&agrave; Qualcomm v&agrave; m&agrave;n h&igrave;nh AMOLED. Ho&agrave;n thiện với chất liệu cao cấp Sở hữu m&agrave;n h&igrave;nh chất lượng với khả năng hiển thị sống động Nổi bật với cụm camera chụp ảnh si&ecirc;u sắc n&eacute;t Hiệu năng tốt sử dụng mượt m&agrave; nhiều t&aacute;c vụ</p>\n', 3, 0.0),
...
ALTER TABLE `tbl_blog_comment`
  ...
  ADD KEY `fk_blog_comment_nguoidung` (`ma_kh`),
  ADD KEY `fk_blog_comment_blog` (`id_blog`);
...
ALTER TABLE `tbl_reply_blog_comments`
  ADD PRIMARY KEY (`id`),
  ADD KEY `fk_reply_blog_comment` (`id_blog_comment`),
  ADD KEY `fk_reply_blog_user` (`id_user`);
...
ALTER TABLE `tbl_reply_comments_product`
  ADD PRIMARY KEY (`id`),
  ADD KEY `fk_reply_comment_product` (`id_comment`),
  ADD KEY `fk_reply_comment_product_user` (`iduser`);
...
--
ALTER TABLE `tbl_blog_comment`
  ADD CONSTRAINT `fk_blog_comment_blog` FOREIGN KEY (`id_blog`) REFERENCES `tbl_blog` (`blog_id`),
  ADD CONSTRAINT `fk_blog_comment_nguoidung` FOREIGN KEY (`ma_kh`) REFERENCES `tbl_nguoidung` (`id`);
...
--
-- Các ràng buộc cho bảng `tbl_reply_blog_comments`
--
ALTER TABLE `tbl_reply_blog_comments`
  ADD CONSTRAINT `fk_reply_blog_comment` FOREIGN KEY (`id_blog_comment`) REFERENCES `tbl_blog_comment` (`id_bl`),
  ADD CONSTRAINT `fk_reply_blog_user` FOREIGN KEY (`id_user`) REFERENCES `tbl_nguoidung` (`id`);

--
-- Các ràng buộc cho bảng `tbl_reply_comments_product`
--
ALTER TABLE `tbl_reply_comments_product`
  ADD CONSTRAINT `fk_reply_comment_product` FOREIGN KEY (`id_comment`) REFERENCES `tbl_binhluan` (`ma_binhluan`),
  ADD CONSTRAINT `fk_reply_comment_product_user` FOREIGN KEY (`iduser`) REFERENCES `tbl_nguoidung` (`id`);
